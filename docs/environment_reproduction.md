...
2. **SOP-08 及后续训练环境**：PyTorch 基线已经验证，但训练入口、最终依赖集合和分布式训练参数尚未冻结。

因此，本文可以可靠复现当前数据管线，并可提前准备 GPU 训练基础环境；它不声称尚未实现的训练流程已经端到端复现。
...
完成标志是“环境与 GPU 基础检查通过，等待训练代码和完整数据 handoff”，不是“训练已复现”。
...
当前仓库中的正式 CLI 覆盖数据准备与风险 shard 生成，尚未冻结 SOP-08 训练入口。因此：

- PyTorch 2.0.1+cu118 是当前通过导入和 ABI 检查的训练基础版本，不是最终训练 lockfile。
- 尚不能承诺另一台服务器上的最终 optimizer、AMP、分布式训练或 checkpoint 完全复现。
- SOP-08 训练入口合入后，必须补充训练 smoke、单 GPU batch、checkpoint round-trip 和固定 seed 数值检查，再将本文训练部分升级为“已验证”。
- 如果训练实现引入新依赖，应同时更新依赖声明和本文件，不得让实际环境继续领先于文档。

在上述门槛完成前，对外表述应为“数据管线环境可复现，GPU 训练基础环境已准备”，不能表述为“完整论文训练环境已复现”。
