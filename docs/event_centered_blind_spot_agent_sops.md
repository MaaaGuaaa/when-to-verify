...
`hidden_risk_occupancy`、`robot_future_footprints` 和
`future_endpoint_times_s` 是 supervision/offline-metric sidecar，不得加入
`model_inputs` 或上述通道表。
...
- [x] 两个 mask 复用冻结的 robot-centric `160×160`、`0.1 m/cell`
      ...
      undeclared/context actor，不按未来可见性裁剪。
      `empty_blind_spot`/无 hidden target 时必须全零
- [x] sidecar 仅作 supervision/offline metrics，不进入 `model_inputs`；
      发布/加载必须同时校验有序 `sample_id`、source risk semantic digest、
      pair completion marker 和 split collection seal，禁止宽松位置 join
- [ ] 对已接受 SOP07 数据完成 `train` / `calibration` / `val` / `test`
      四 split sidecar backfill，每 split 发布 authenticated seal，并纳入新完整训练包；
      不覆盖已发布 risk shards 或旧 bundle
...
- 四 split 每个已接受 risk shard 都有唯一 sidecar + marker，无
  missing/extra/duplicate ID，source semantic digest 全部匹配
- 正式 loader 全量验证 shape/dtype/finite/binary、端点时间、全零边界；
  四个 authenticated seals 均可重载
- 新完整 bundle 包含四 split risk、sidecars/markers、seals、manifest 和逐文件
...
- sidecar 仅在 label/offline-metric 分支出现，不进入 baseline `model_inputs`
