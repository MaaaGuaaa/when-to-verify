...
BaseState z
    ↓
读取 schema 3 候选轨迹 ξ，缓存机器人 history+future 连续扫掠体
    ↓
...
正式生产算法标识为 `blind_reachability_first_v1`（项目流程中亦称 v5）。
“v5”是 SOP05 流程代际，不是全局 schema 版本；所有产物仍必须标记
...
`blind_reachability_first_v1`。它不先生成目标曲线再反复尝试用单一正前方障碍遮住，
...
`blind_reachability_first_v1` 正式 producer 的 `production_event_kind` 固定为
...
- 每个 `risk_shard_npz_jsonl_v1` immutable shard 目录精确包含 `samples.npz`、`metadata.jsonl` 和
...
这些模块和分片契约已经有单元/确定性验证，但不等于整条数据集生成任务
已完成。正式 `scripts/04_generate_risk_dataset.py` 编排入口、10–100 真实样本 smoke
与 50k/240k 目标规模运行仍待验证，未验证前不得宣称达到该规模。
...
下列命令是目标编排顺序，不表示所有 CLI 已实现。当前 SOP07 的 GT、
`RiskSample` 和 immutable shard API 已实现，但正式 `04_generate_risk_dataset.py`
编排入口仍待完成和真实数据 smoke 验证。
...
# 5. 生成事件中心风险数据（planned；尚未做 publication-scale 验证）
