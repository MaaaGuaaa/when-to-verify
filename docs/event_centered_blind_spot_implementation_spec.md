...
5. 对真实动态对象轨迹片段只做刚体变换，并在固定真实时间网格内选择冲突锚点，使其从盲区出现并在指定时刻与机器人轨迹交互；
6. 同一场景生成碰撞、近碰、时序安全、空间安全、无关隐藏对象、空盲区等成对样本；
...
其中 K 个历史采样点对应 `t-1.4, t-1.2, ..., t`；`1.6 s` 是 8 个
`0.2 s` 时间 bin 的名义窗口宽度，首末采样点的实际时间跨度为 `1.4 s`。主生产
`MotionSnippet` 必须一次覆盖真实历史、当前和未来，冻结局部布局为：
...
current_index                 = 7
future_steps                  = 15
relative_times_s              = [-1.4, -1.2, ..., 0.0, 0.2, ..., 3.0]
```

`poses[0:8]` 是真实历史，`poses[7]` 是当前，`poses[8:23]` 是未来。禁止重复当前
位姿、反向外推或用理想轨迹补齐任一侧。该布局是 snippet artifact 的局部契约，不修改
全局 `SCHEMA_VERSION`；旧 16 点/3.0 s library 不做静默兼容。
...
observed `BaseState` 与完整 `OracleContext` 必须写入不同对象/文件。对象 ID 排序且唯一；history/future/spec 的 key 必须严格对齐。`BaseState` 不得携带 future。schema v1 产物不做兼容读取，升级后统一重建。
...
避免用纯随机直线动态体。真实对象轨迹片段保留速度变化、轻微弯曲和加速度统计；主生产程序只做 SE(2) 变换，不做时间缩放。片段按 split 和 `object_type` 分库。
...
主生产 library 只接受上述固定布局。时间窗不完整、跨越过大 gap 或不足 23 个真实重采样
点的候选必须明确拒绝；不得补点或外推。4.4 s 连续窗口会改变各 split/type 的候选量，
因此必须重新报告 candidate/accepted/rejected 数量和拒绝原因。SOP03 不为下游增强额外
扩展 source window；不得生成 28 点/5.4 s 主生产变体。
...
MotionSnippet = {
    ...
    "object_type": str,
    "footprint": dict,
    "positions": np.ndarray[23, 2],
    "velocities": np.ndarray[23, 2],
    "headings": np.ndarray[23],
    "sample_dt_s": 0.2,
    "history_steps": 8,
    "current_index": 7,
    "future_steps": 15,
...
本文其余位置使用的 `poses[23,3]` 是
`concatenate([positions, headings[:,None]], axis=1)` 的逻辑视图，不是 artifact 中另一个
可独立漂移的数组字段。

artifact summary 和 manifest 必须重复记录 layout version、23/8/7/15、`0.2 s`、
`4.4 s` 与冻结 split digest；loader 对缺字段或布局不一致必须失败。

### 6.3 允许变换

\[
\tilde h_\tau=R(\phi)h_\tau+t
\]
...
time scale a：主生产固定为 1.0
额外位置噪声：最多 0.05m，默认不用
```

主生产配置中的 `time_scale_range` 必须规范化为 `[1.0, 1.0]`；其他范围立即报契约错误。
`a=1.0` 表示保留真实片段的速度、加速度和时间间隔。collision 与 temporal-safe 的时序
差异通过在同一条 23 点真实片段内选择不同冲突锚点实现，不得用时间重采样替代。
...
- 速度符合对象类型配置范围；
- 加速度不超过阈值；
- 当前帧满足不可见约束；历史帧按请求的事件类型允许真实可见或隐藏；
- 未来窗口内至少部分进入局部地图；
...
放置遮挡物或施加 FOV 盲区
    ↓
按对象类型抽取真实 MotionSnippet
    ↓
变换 snippet，使所选动态对象在 τ* 附近通过 p*
    ↓
生成 paired variants
    ↓
渲染历史观测与 oracle 世界
...
`0/0.25/0.5/0.7`，并把 conflict-time quantile 固定为 `1.0`（配置窗口内最晚的可用
离散时刻）。八个 `(offset, angle)` 组合按事件 seed 排列；每个组合先尝试法向 `-1`
侧，再紧邻 `+1` 侧 fallback，共 16 个候选。time-scale quantile 不再是候选维度，所有
候选使用 `a=1.0`。前缀失败后才进入覆盖
...
上述 v2 名称只标识冻结的候选调度前缀。包含完整对象扫掠和正式发布边界的 SOP05
母事件生成协议固定为 `joint_occluder_first_v4`，并写入 event、world、pair report 和
publication metadata。producer 与正式 loader 必须精确校验该值；旧 v3 及更早产物需
重新生成，不得并行兼容或在加载时升级解释。

程序化遮挡物在完整 `K+T` 时间窗内视为固定几何。它必须同时避开基础静态图、
`robot_history[K]`、机器人当前和未来 swept volume、所有上下文对象的完整
history/current/future footprint，以及 collision/temporal-safe 目标的完整
history/current/future footprint；不能只检查当前帧或未来轨迹。

所有动态 sweep 的碰撞检查不能直接用栅格掩码相交作为最终判据。实现必须显式构造
机器人 `history+future`、每个上下文对象 `history+future` 和目标对象
`history+future` 的 23 帧 sweep；传感器当前位姿只能用于 LOS/FOV，不得隐式前插到
碰撞 sweep。每条 sweep 先按不超过半个 BEV cell 的平移步长和 `5°` yaw 步长加密
SE(2) 轨迹，再用真实 footprint signed clearance 做窄相检查；相邻加密点之间用
“平移距离 + footprint 外接半径乘 yaw 变化”的保守运动上界认证整段安全。若区间不能
被证明为正净空，则递归检查中点并在精度上限处保守拒绝。这样既不会因栅格接触误拒绝
真实正净空，也不会漏掉任意对象的帧间窄碰撞。
...
同时存在 FOV 限制和实体遮挡。第一版默认 proposal distribution 为：
...
上述 `60/30/10` 只控制 environment/structural/mixed 的生成提议分布，并作为类型计数的
审计基准，不是 publication quota。发布时从全部满足碰撞、当前遮挡、未来出现和物理
有效性契约的事件中，按由 seed 与 `generated_event_id` 派生的稳定全局 selection key
排序并选取请求的总 quota。`event_kind` 不影响事件的发布资格、全局排序或 complete
判定；各类型 generated/selected 数量仅用于诊断。物理合法事件总数不足总 quota 时才
标记 `quota_unmet`，否则发布为 `complete`。
...
主生产 snippet 的 source current 固定为 `current_index=7`，对应 source time
`(K-1)dt=1.4 s`。给定输出侧未来冲突时刻
`conflict_time_s = τ_output*`，source 冲突锚点时间必须为：

\[
t_{source}^*=1.4+\tau_{output}^*
\]

在该 source time 选择/插值冲突锚点 \(k^*\)，要求变换后：

\[
\tilde h_{k^*}\approx p^*
\]

并满足 source/output 时间关系：

\[
t_{source}(k^*)-1.4\approx\tau_{output}^*=\texttt{conflict\_time\_s}
\]

禁止继续直接用 `conflict_time_s` 作为 source time，否则 history/current/future 会整体
错位 `1.4 s`。变换后的生成阶段对象保存：

```text
history_poses: float32[K, 3]
future_poses:  float32[T, 3]
current_pose = history_poses[K - 1]
```

`OracleWorld.dynamic_object_trajectories` 仍只保存 `[T,3]` future，因此无需修改冻结的
`src/contracts.py`。SOP05→SOP06 的正式阶段边界冻结为
`event_target_motion_history8_future15_v1`。版本化 generated-event shard 必须保存：

- `generated_event_id`、`world_id`、`base_state_id`、`trajectory_id`；
- `target_dynamic_object_id`、`source_snippet_id`、`target_type_policy_digest`；
- `target_history_poses: float32[8,3]`、`target_current_pose: float32[3]`、
  `target_future_poses: float32[15,3]`；
- footprint spec、layout version、两段数组 digest 和 manifest digest。

上述 identity 必须与 event manifest 及 OracleWorld 的字段/metadata 逐项一致，且一个
`generated_event_id` 恰好对应一条 target-motion record。loader 必须验证 finite、
shape/dtype、`target_history_poses[7] == target_current_pose`，且 future 第一帧对应
`+0.2 s`；还必须逐元素校验
`target_future_poses == OracleWorld.dynamic_object_trajectories[target_dynamic_object_id]`、
footprint spec 相等并核对 digest。OracleWorld 只消费 future，renderer 只消费 history；
禁止依赖不可序列化的临时 Python 对象跨阶段传递。
...
1. 校验 snippet 是 23 点冻结时间网格且 `time_scale=1.0`，不做时间重采样；
2. 将 \(h_{k^*}\) 平移到原点；
3. 将 \(\dot h_{k^*}\) 旋转到目标横穿方向；
4. 平移到 \(p^*\)；
5. 用同一个刚体变换生成完整 `history_poses + future_poses`；
6. 检查完整历史/未来轨迹、逐帧 yaw 和真实 footprint。

任何冲突锚点或 temporal-safe 偏移若需要访问 snippet 的 `[0,4.4 s]` 之外都必须拒绝，
不得外推。过去某些帧允许目标可见，以保留真实 last-seen/age 线索；所有主事件仍必须
保证当前帧目标不可见。变换后的 history/future 接缝也必须参与速度、加速度和 yaw 连续性
检查。
...
- 当前时刻目标对象可见；
- 目标历史缺失、布局不是 `history8_current7_future15_v1`、`time_scale != 1.0`，或所请求
  冲突锚点超出真实 23 点窗口；
- 过去历史可见性不符合请求的目标样本类型；
...
未来出现”，默认以目标当前隐藏位姿为枢轴，对完整 23 点 history/current/future 轨迹和逐帧 yaw 做
...
环境遮挡母事件不能先冻结单一当前视线再独立重采样 temporal-safe。完整配对搜索应先按
“可用正时间偏移最大、冲突时刻最晚”排序离散冲突锚点；每个锚点连续消费
`joint_occluder_first_v2` 的完整 16 候选高可行前缀，避免短批次反复重置 seeded 前缀。
找到同步安全且空间路径仍相交的 temporal target 后，用同一个配置内矩形遮挡物同时覆盖
collision 与 temporal target 的两条当前 LOS；候选还必须同时通过机器人 swept volume、
基础静态图、上下文对象和两条目标 footprint 轨迹的净空检查，并对两条目标序列分别
验证“当前隐藏、未来连续出现”。完整 history/future 扫掠检查必须位于同一个 multi-LOS
几何候选内层循环中；某个候选碰撞时继续枚举同一 normal offset 的后续尺寸、朝向和中心
位置，不能直接放弃整个 offset。只有该共同遮挡骨架能生成完整 six-pack 时才接受；失败
原因、母候选数、配对候选数和遮挡候选数分别进入 summary。

正式 SOP06 环境 six-pack consumer 必须精确要求 paired generator
`joint_environment_pair_v2` 和 multi-LOS placement
`joint_multi_los_envelope_v2`。即使目录或 metadata 被整体重封且内部自洽，v1、缺失或
未知版本仍必须拒绝；禁止同时兼容两套几何语义。
...
同一场景只移除 `target_dynamic_object_id`：从 renderer scene history/spec/current pose
和 OracleWorld future/spec 同时移除该目标，其他动态对象的历史、未来与 specs 必须保持
不变。删除后必须从 target-removed scene 重新运行 renderer，重新计算
`past_dynamic_occupancy`、last-seen、age 和所有 current state channels；不得复用 collision
variant 的渲染结果。供 paired audit 使用的 removed-target provenance 可以保留在模型
输入之外的 manifest。
...
完整环境 six-pack 的请求级成功率和内部候选接受率必须分开报告；前者不得替代
SOP-05 原始事件生成器的 `attempt_acceptance_rate`，也不得用只保留成功组的方式隐藏
重采样成本。
...
不得复用它来覆盖上下文对象。稳定摘要按层级冻结：

- `snippet_id` 纳入 layout version 和源 23 点 positions/headings 数组 digest；
- `target_dynamic_object_id` 纳入共享的 base/trajectory/source snippet/mother seed 身份，
  不纳入 variant-specific history；
- `pair_group_id` 纳入共享场景骨架、source target 和母事件配置，不纳入 variant-specific
  history/future；
- mother event ID 只纳入母事件自身的 history/future digest；
- world ID 与 variant/sample ID 纳入对应 variant 的 history/future digest。

因此 six-pack 的 target ID 与 pair ID 保持相同，而不同 variant 的 world/sample ID 不同。
...
核心 renderer 的接口只接收 `BaseState`、`scene_dynamic_history[K]`、对应 footprint specs、
当前静态/程序化遮挡 occupancy、blind-spot sensor config 和基础配置。它不得接收
`OracleWorld`、`OracleContext`、`dynamic_object_future` 或任何 future trajectory。
`scene_dynamic_history` 可包含完整上下文历史和移植目标的真实 `history_poses`，但输出前
必须逐帧经过传感器可见性净化。在 BaseState、scene history/spec、静态/程序化
occupancy、sensor config 全部固定时，只改变 dynamic future trajectories 或 future
dynamic occupancy，不得改变 renderer 输出的任一元素或安全 metadata。
...
3. 按每个上下文对象与移植目标的真实 pose/yaw 和 circle/rectangle spec 放置历史 footprint；
...
隐藏 actor 的真实历史 footprint 可以进入 ray casting 内部的 `total_occupancy`，用于正确
形成遮挡关系；只有 `dynamic_occupancy & visible_mask` 可以写入
`past_dynamic_occupancy`。目标过去可见的帧可以进入 last-seen 和 age map，隐藏 cell 的
actor occupancy 不得进入任何输出 channel、metadata 或调试字段。当前主事件的目标必须
不可见。renderer 输出只包含冻结的 BEV/state arrays 和安全 provenance，不得包含目标
pose、对象轨迹、world ID 或可反查 hidden history/future 的字段。
...
通道语义冻结为：`current_visible_occupied` 是当前可见的 static + dynamic occupancy；
`last_seen_occupancy` 是每个 cell 最近一次可见时的 dynamic occupancy，从未见过的 cell
取 `0` 并由 `occlusion_age_map=1` 表示未知；`static_obstacle_map` 是完整已知的
static/programmatic occupancy，不受当前可见性裁剪；robot velocity 与 yaw-rate scalar
在各自通道内全图广播。任何变更都属于输入 schema 语义变更。

renderer metadata 白名单只允许 `renderer_layout_version`、`base_state_id`、
`sensor_config_digest` 和 `static_occupancy_digest`；不得包含生成时间、world/target ID、对象
列表或 visibility truth。robot velocity/yaw-rate 使用原始 SI 单位并以 float32 广播。
...
python scripts/02_build_snippet_library.py --split train --duration-s 4.4
python scripts/02_build_snippet_library.py --split calibration --duration-s 4.4
python scripts/02_build_snippet_library.py --split val --duration-s 4.4
python scripts/02_build_snippet_library.py --split test --duration-s 4.4
