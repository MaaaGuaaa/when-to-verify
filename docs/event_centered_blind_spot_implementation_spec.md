...
正式生产算法标识为 `blind_reachability_first_v2`（项目流程中亦称 v5）。
“v5”是 SOP05 流程代际，不是全局 schema 版本；所有产物仍必须标记
`schema_version=3.0.0`。

v2 同时冻结 `reachability_candidate_se2_v2`：`ReachabilityIdentity` / candidate
...
`blind_reachability_first_v2`。它不先生成目标曲线再反复尝试用单一正前方障碍遮住，
...
`normal_offset_range`。该块已废弃，不属于 v2 合法参数；当前位姿只能来自
`causal_free_space_schedule_v1` 的 interaction-range / bearing-bin / type-size-yaw
枚举与后续 causal-delta 验证。禁止根据法向偏移公式在 v2 中恢复
单一“正前方障碍”模式。
...
`blind_reachability_first_v2` 正式 producer 的 `production_event_kind` 固定为
